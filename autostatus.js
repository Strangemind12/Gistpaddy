...
async function autoStatusCommand(sock, chatId, msg, args) {
  try {
    ...
    if (command === "on") {
      ...
      return sock.sendMessage(chatId, {
        text: "✅ Auto forward on status reply enabled!",
        ...channelInfo,
      });
    } else if (command === "off") {
      ...
      fs.writeFileSync(configPath, JSON.stringify(config));
      return sock.sendMessage(chatId, {
        text: "❌ Auto forward on status reply disabled!",
        ...channelInfo,
      });
    } else {
      return sock.sendMessage(chatId, {
  ...
  try {
    ...
    if (update.messages && update.messages.length > 0) {
      ...
      if (
        ...
        if (!statusOwner)
          return console.log("⚠️ Could not determine status owner");
