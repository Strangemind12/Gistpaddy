...
const cleanTempFolderIfLarge = () => {
  try {
    ...
    if (sizeMB > 100) {
      ...
      console.log("👑 Storage: Cleared temporary media cache");
    }
  } catch (err) {
  ...
  try {
    ...
    if (mediaType) {
      ...
      const fileName = `${messageId}.${
        mediaType === "sticker" ? "webp" : mediaType === "video" ? "mp4" : "jpg"
      }`;
  ...
  try {
    if (!message.key.fromMe) {
      return sock.sendMessage(chatId, {
        text: "👑 *Decree:* Only owner can adjust these settings!",
    ...
    if (!match) {
      return sock.sendMessage(chatId, {
        text: `*👑 QUEEN MARVEL MD — ANTIDELETE SETTINGS 👑*\n\nCurrent Status: ${
    ...
    if (match === "on") {
      ...
      return sock.sendMessage(chatId, {
        text: "👑 *Announcement:* Antidelete protection has been activated",
      });
    }
    ...
    if (match === "off") {
      ...
      saveAntideleteConfig(config);
      return sock.sendMessage(chatId, {
        text: "👑 *Announcement:* Antidelete protection has been deactivated.",
      });
    }

    return sock.sendMessage(chatId, {
      text: "👑 *Notice:* Invalid command. Use *.antidelete* to see options.",
    });
  } catch (err) {
    console.error("👑 Command Error:", err);
    await sock.sendMessage(chatId, {
      text: "👑 *Notice:* An error occurred while processing your command.",
    });
  }
}
...
async function handleMessageRevocation(sock, message) {
  try {
    ...
    let text =
      `*👑 QUEEN MARVEL MD - ANTIDELETE REPORT 👑*\n\n` +
    ...
    if (original.mediaType && fs.existsSync(original.mediaPath)) {
      const mediaOptions = {
        caption: `*👑 Deleted ${original.mediaType} restored by Queen Marvel MD*\nFrom: @${senderName}`,
        mentions: [sender],
      };
      ...
      try {
        ...
        await sock.sendMessage(ownerNumber, {
          text: `⚠️ Queen Marvel MD encountered an error sending media: ${err.message}`,
        });
      }
